...
保存先を切り替えられる。元は juice 内の `shoudou_storage` で、独立ライブラリ `manystore`（別 repo）として
抽出された。

**スコープ:** ストレージ抽象とその backend 実装・接続・安全パス・合成ストアまで。これを超える利用側
（juice 等）固有の要求は manystore 本体に取り込まず、利用側の adapter で吸収する（下記コア要件参照）。
...
- **pristine に保つ**：利用側都合で IF を安易に拡張しない（最小プリミティブから合成）。

## ゴール

- juice が「利用する側」として manystore の backend を adapter 経由で使えること。
- 単体で `uv run pytest` が緑（独立ライブラリとして自己完結）。
- 実 backend（minio / 実 NATS）での疎通まで検証できること。

...
| M078 | 横断関心のミドルウェア合成層（logging/retry/metrics/cache） | 相談 | ユーザー提案（2026-07-02）＝継承/ラップで機能追加する代わりに順序付きミドルウェアを install して op を挟む（ASGI 風）。**適用範囲の線引きが肝**＝(a) **横断的・任意の関心（logging=M015 / op-retry=M014 / metrics / tracing / cache）はミドルウェア向き**（ラッパ乱立回避・productContext「ネスト禁止＝挙動が割れる」の健全な代替＝順序が明示された合成）／(b) **Safe(必須パス検証) と基底の buffered↔stream 合成は対象外**（前者は安全不変条件で opt-out 不可・後者は型の一部）。**設計コスト**＝Store は ~15 の異種メソッドで ASGI の 1 本呼び出しほど綺麗にならない＝per-method フック（薄い・関心少なら十分）か op 封筒（汎用だが型安全喪失）の二択。M071 と直交（統合 Store の上に載る合成層）＝M071 完了後に doc-first。M014/M015 を包含 |

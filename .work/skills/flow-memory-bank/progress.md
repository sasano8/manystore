...
- **2 方向の汎用アダプタ**（`async_storage`）: `KeyValueFileStore`（KVS→FileStore）/ `KeyValueFromFileStore`
  （FileStore→KVS）。**local は後者で構成**＝`LocalFileStore` が filesystem-native な真実の実装（put/get・
  iter/list/exists/delete・vacuum・cp/mv・open_reader/open_writer）で、`LocalKeyValueStore` は
  `KeyValueFromFileStore(LocalFileStore)` の薄い KVS ビュー（M027）。`FileStore` Protocol は名前空間操作まで含む。
...
| M022 | ストレージ適合性テスト（conformance / contract test suite） | **未着手・設計先行（バックログ）** | **supervisor interrupt（2026-06-23, normal）**＝`interrupt/20260623-storage-conformance-test-suite.md` を取り込み。共通 IF（`KeyValueStore`/`FileStore`）の backend 非依存な振る舞い契約をテストとして固定し、**任意の backend（外部第三者実装含む）が接続情報を差すだけで「IF 準拠か」を証明できる再利用可能スイート**にする。要点: 既存 `tests/test_storage.py`/`test_e2e_backends.py` を契約スイートとして抽出・共通化できるか整理（重複増やさない＝projectbrief）／パラメタ化単位（backend factory・接続注入）／契約境界（必須準拠 vs optional）。受け入れ: local/nats/s3 で同一契約スイート緑＋外部実装が接続情報を差すだけで回せる形＋実 backend（SeaweedFS/実 NATS）疎通。**flow の deep think 着手前ゲートで設計先行**。dotfiles 側のオープンテスト“プラットフォーム”（M010）とは別タスク（プラットフォームは持ち込まない）。優先 normal（M019/M020 を止めない） |
...
| M027b | **FileStore Protocol 拡張の波及整理**（S3/NATS/HTTP/Safe/Sync FileStore の名前空間操作） | **未着手・設計先行（相談）** | M027 の follow-up。FileStore Protocol に iter/list/exists/delete/cp/mv/connect/aclose を足したが、`S3FileStore`/`NatsFileStore`/`HttpFileStore`/`SafeFileStore`/`SyncFileStore` はこれらを未実装＝**Protocol を部分的にしか満たさない**（CI に型チェッカが無く破綻なし・呼べば AttributeError）。選択肢: (i) 各 FileStore に名前空間操作を備える（S3/NATS は KVS 兄弟のロジックを流用、HTTP は list/iter 非対応・mutate は read-only）／(ii) narrow な内部 Protocol に分け FileStore 本体は最小に戻す。`KeyValueFromFileStore` を Local 以外の FileStore に被せて KVS 全機能を出したい需要が出たら着手。projectbrief「最小・汎用/YAGNI」と要相談 |

...
| M022 | ストレージ適合性テスト（conformance / FileStoreTester） | **P1 メソッド存在＋P2 run_light 完了 / run_middle/heavy/full・spec 検出・リプレイは残** | **ユーザー要望（2026-06-23, 対話）**。**当初の loose な `check_*_contract` 関数版はユーザー指摘でオラクル方式の `FileStoreTester` に作り直した**（旧 check_* は廃止）。<br>**P1 メソッド存在**: `assert_key_value_store`/`assert_file_store`/`assert_implements`/`missing_members`/`required_members`（`typing.get_protocol_members`）。ruff は型/Protocol を検査できない（linter/formatter）ので実行時 conformance で代替。<br>**P2 `FileStoreTester`（2026-06-23）**: `FileStoreTester(reference=DictFileStore, target)`＝**辞書ストアを正（オラクル）**とし同一操作列を両者に適用して観測一致を観点ごとに記録。`run_light` < `run_middle` < `run_heavy` < `run_full` の段階実行。**run_light 実装済**＝open_reader/open_writer/exists/**list_all** を **10 観点**で差分検証。`delete_all`（iter+delete でクリーン化＝ジェネシス・light では検証対象外）。結果は `result()`/`save_json(path)`＝観点ごと `op`/`args`/`expected`/`actual`/`passed` を JSON 保存（**操作ログ＝将来リプレイの素材**）。`tests/test_conformance.py` で Local を辞書オラクルに run_light・自己一致・差分検出・JSON 保存を検証。e2e の KVS ラウンドトリップは `_crud_roundtrip` のまま（run_light は FileStore 専用なので別レイヤ）。`make check` 緑（**107 passed, 1 skipped**）。正本 `docs/architecture.md`。<br>**残（M022b 参照）**: run_middle/heavy/full（cp/mv/list/iter/get・並行・大容量）／spec（file/kv 寄り）自動検出＝特性表／リプレイ実行エンジン／シグネチャ検査／実 backend（S3/NATS FileStore）への適用。原指示 `interrupt/archive/.../storage-conformance-test-suite.md`（supervisor, normal）。優先 normal |

...
| M071 | 公開 IF 統合＝旧 KeyValueStore/FileStore を 1 Store へ | 完了(2026-07-03) | **公開面を完全に 1 Store 化**（`AsyncStore`＝値 API(put/get…)+IO API(open_*)・put/get だけの view は `AsyncBufferedStore`）。Stage 1〜6＝(1)backend 1 クラス化（`DictStore`/`LocalStore`/`S3Store`/`NatsStore`/`HttpStore`/`RemoteStore`・native 能力を `BufferedStoreBase`(値寄り)/`StreamingStoreBase`(IO 寄り)/`StreamableBufferedStoreBase`(両軸 native=S3) の 3 基底で型に出す)(2)`BackendSpec` 単一 factory＋`create_unsafe_store` 一本 (3)2 方向アダプタ(KeyValueFileStore/KeyValueFromFileStore)撤去＝合成は基底の既定へ (4)公開型 `AsyncStore`＋統合 facade `manystore.store` (5)M073 spec/impl 分離(`manystore/spec/`＝protocols/base/exceptions・conformancer 同居) (6)docs 更新＋**語彙 purge**＝旧「KeyValueStore/FileStore」概念を src/tests/README/examples/生成 docs から一掃・合成/conformancer も Store 名へ(SafeStore/ArrayStore/StoreTester/assert_store/assert_buffered_store)・**後方互換 shim なし全面撤去**。設計=(削除済)plan・詳細経緯は git 履歴。make check 緑(268)・test-heavy 緑(44・実 backend)・mkdocs --strict 緑 |

...
| M027 | **Local の KV を FileStore から派生**（`KeyValueFromFileStore`・メイン実装を `LocalFileStore` に集約） | **未着手・設計先行（相談）** | **ユーザー要望（2026-06-23, 対話）**。現状 `LocalKeyValueStore` と `LocalFileStore` は別々の実装（各々が resolve/mkdir/atomic write を二重に持つ）。要望＝**`LocalFileStore` を真実の実装にし、`LocalKeyValueStore` はそこから派生したバッファ KV ビュー**にする。既存 `KeyValueFileStore`（KVS→FileStore）の**対称な逆 `KeyValueFromFileStore`（FileStore→KVS）**を新設し `KeyValueFromFileStore(LocalFileStore(...))` で local KV を構成。<br>**設計の壁**: `FileStore` Protocol は open_reader/open_writer のみで **list/exists/delete/iter/cp/mv が無い**＝`KeyValueFromFileStore` は get/put しか派生できない。メイン実装を LocalFileStore に寄せるには (a) FileStore を list/exists/delete で拡張、(b) LocalFileStore が Protocol 超のメソッドを持つ、(c) Local は metadata 操作用の薄い KVS を残し IO のみ委譲、のいずれか要決定。projectbrief「最小・汎用/YAGNI」と緊張＝**doc-first 合意してから着手**。<br>背景方針（バッファ性）は systemPatterns 原則6・本表 M025/M026 と連動。詳細 `interrupt/archive/2026-06-23-local-kv-from-filestore-and-buffer-semantics.md` |

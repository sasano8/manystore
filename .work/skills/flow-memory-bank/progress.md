...
| ID | タスク | 優先 | 備考 |
|----|--------|------|------|
| ~~M046残~~ | ~~conditional put の残~~ → **完了（2026-06-28）** | done | **全面完了**。コア backend（local/dict/S3/NATS）＋ native REST＋remote＋**S3 ゲートウェイ**まで conditional put/CAS を配線。conformancer で create/update CAS の並行安全性を機械検証（local/nats は実 backend e2e でも緑）。設計 `plans/m046-conditional-put-plan.md`（完了 plan は GC 候補）。完了詳細は下記マイルストーン |

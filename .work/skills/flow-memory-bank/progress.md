...
| M030 | **prefix を capability に移設**（`SupportsPrefixListing` + 汎用ヘルパ） | **未着手・設計確定（2026-06-24）** | **ユーザー要望（2026-06-24, 対話）**「prefix が S3 互換なら backend/ラッパーに実装したい」。prefix は NATS 由来ではなく `service.list_entries` の汎用 startswith フィルタ。これを **core IF 最小のまま optional capability に移設**＝`SupportsPrefixListing` Protocol（`iter_prefix(prefix)`）＋汎用フォールバックヘルパ `iter_prefix(store, prefix)`（ネイティブが有ればそれ・無ければ `iter_all()`+startswith 総なめ）。**S3=ネイティブ**（`list_objects_v2(Prefix=…)`＝サーバ側で絞る）／NATS/HTTP/local/dict=フォールバック。**伝播**: `SafeKeyValueStore`/`ArrayKeyValueStore` が `iter_prefix` を委譲して native 効率を通す（**M027b と同じ capability 伝播パターン**。委譲が無いと S3 native が隠れる）。使用元差し替え＝**S3 ゲートウェイ ListObjectsV2** と **multipart 内部の part 列挙** を `iter_prefix` ヘルパ経由に。native HTTP からは prefix 撤去（M025改）。詳細 `m025-namespace-restructure-plan.md`。priority normal |

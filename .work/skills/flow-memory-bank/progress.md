...
| M065 | conformancer を「実装漏れ検知器」に育てる（run_middle） | **進行中（high）** | **ユーザー方針＝実装漏れは conformancer に契約として実装し横断検知**。**step1 完了（2026-06-28）**＝① `run_middle` 本実装（delete・欠損 delete 冪等・複数キー列挙順・read 境界・overwrite 縮小の差分観点＋`_op_delete` 追加）／② オラクルで表せない**絶対契約** `assert_writer_aborts_on_error`（writer の all-or-nothing＝M058 を横断検知。dict も同 writer 共有ゆえ差分では捕まらない）を新設し、牙テスト（committing writer が落ちる）付き。これで M058 を契約先行で修正・緑化。conformance test 25→30。**残（step2+）**＝(a) get_or_raise の欠損のみ NotFoundError・他伝播の契約／(b) exists と get の整合／(c) **fault-injecting ラッパストア**を同梱して 5xx 伝播等（M054/M055 クラス）を wrapper-under-test に被せ検証／(d) 非 CAS 並行（並行 delete/get）／(e) run_heavy/full。新 backend は run_middle＋絶対契約を通すだけで実装漏れが loud に落ちる状態を目指す |

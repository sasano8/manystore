...
- **適合性ツール（M022 フェーズ1）**: `manystore/conformance.py`（`assert_key_value_store`/`assert_file_store` 等＝
  Protocol メソッド存在チェック）。サードパーティ backend が pytest から横断的に準拠確認できる。挙動契約は未実装。
...
| M022 | ストレージ適合性テスト（conformance / contract test suite） | **フェーズ1（メソッド存在チェック）完了 / 挙動契約は未実装** | **ユーザー要望（2026-06-23, 対話）で着手**＝「サードパーティ backend が簡単に使える仕様テスト。前提 Protocol メソッドの存在を横断確認。挙動一致は開発途上なので未実装でよい」。新 `manystore/conformance.py`（`assert_key_value_store`/`assert_file_store`/`assert_implements`/`missing_members`/`required_members`＝`typing.get_protocol_members` で Protocol メンバを取り callable 属性として在るか検査）＋ `tests/test_conformance.py`（全 backend を横断チェック＋FileStore⊇KVS の包含確認＋ツール健全性）。ruff は型/Protocol 準拠を検査できない（linter/formatter）ので実行時 conformance で代替。**残（契約スイート）**: 挙動の一致（同一 CRUD を backend factory 注入で回す＝既存 `tests/test_storage.py`/`test_e2e_backends.py` から抽出・共通化できるか／パラメタ化単位＝backend factory・接続注入／必須準拠 vs optional の契約境界）・シグネチャ検査・実 backend（SeaweedFS/実 NATS）疎通。受け入れ=外部実装が接続情報を差すだけで回せる形。原指示 `interrupt/archive/.../storage-conformance-test-suite.md`（supervisor, normal）。dotfiles のオープンテスト“プラットフォーム”（M010）とは別。優先 normal |

...
| M075 | CLI を Typer へ移行 | normal | ユーザー要望（2026-07-02）＝現 `__main__.py` の argparse サブコマンド（serve / store init）を Typer に置換。依存 `typer` を追加（CLI extra `[cli]` か server extra へ相乗り＝要判断・遅延 import で `import manystore` を壊さない）。**後方互換**＝旧 `manystore --config`（先頭フラグ→serve 振り分け）を維持するか、`serve` 必須に切るか要判断。`store init`/`serve` の help・型・補完が改善。conformancer/server/gateway の各 `__main__` も Typer 化するかは別途 |

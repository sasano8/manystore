...
- **統合エントリポイント（M023）**: `manystore.combined.create_combined_app` / `python -m manystore`。
  `/manystore`=native REST/WS・`/s3`=S3 ゲートウェイを `include_router(prefix=...)` で 1 アプリに束ね、
  共有 service を単一 lifespan で 1 回だけ connect/aclose。S3 クライアントは `endpoint_url=<host>/s3`。
  単体アプリ/単体起動は後方互換で維持。`make check` で **91 passed, 1 skipped**。

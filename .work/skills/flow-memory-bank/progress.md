...
| ~~M063~~ | ~~`DownloadCache.download` の同期 IO offload~~ → **完了（2026-06-28）** | done | async 内で直接実行していた同期 FS 操作（`is_file`/`mkdir`/`_atomic_write_bytes`）を `anyio.to_thread.run_sync` でスレッドへオフロード＝event loop を塞がない（local backend と同流儀）。stat と「mkdir＋原子書き込み」を各 1 スレッドホップに。`make check` 緑（184） |
...
| M066 | 挙動契約を全 backend＋client 横断で実行（集約ハーネス） | **進行中（high）** | **ユーザー指摘＝IF が揃うので同一契約 body を複数実装に流せるはずだが配線が偏っていた**。**step1**＝Remote/実 backend へ contract を個別配線。**step2 完了（2026-06-28・集約）＝1 パラメタ化ハーネスへ集約**＝`tests/conformance_providers.py` に **全 provider を 1 か所宣言**（dict/local/remote/nats/s3-virtual/s3-path。各 `open()`→接続済み FileStore・`gated`・`isolated`〔delete_all 可〕・`reachable`。KVS-native は `KeyValueFileStore` で FileStore 化）＋`tests/test_conformance_matrix.py` が全契約を流す＝**非破壊**（CRUD/writer/並行 CAS×絶対）は全 provider・**破壊的差分**（run_light/run_middle＝delete_all 全消去）は isolated（dict/local/remote）のみ。実 backend は gated（未到達 skip・slow・`make e2e-up`→`make test-heavy`）。重複を集約先へ移し削除＝`test_e2e_backends.py` 廃止（openers→providers・crud/cas→matrix）／test_conformance の Dict/Local writer・並行を削除（ツール牙/winner 識別は残置）／test_client の Remote 系 5 テスト削除。fast 176 passed・gated 12 skip。**Remote の fail-loud は M065 step5 で閉じた**（500 fault transport の RemoteKeyValueStore に `assert_fail_loud_over_transport` を当て全 op の非握り潰しを契約化）。**残**＝① 実 backend への run_light/run_middle（専用 NS or 非破壊版 run_* の設計＝delete_all 全消去回避）／② leaf backend（nats/s3）の fail-loud（backend 固有の fault transport が要る）／③ native FileStore writer の直接検証（現状 KVS バッファ writer 経由） |

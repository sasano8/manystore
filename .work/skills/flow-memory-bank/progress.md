...
| M065 | conformancer を「実装漏れ検知器」に育てる（run_middle） | **high** | **ユーザー方針（2026-06-28）＝実装漏れ系のバグは個別 test でなく conformancer に契約として実装し、全 backend 横断で機械検知したい**。run_light（12 観点）の上に **run_middle** を立ち上げ、監査で出たクラスの細かい契約を積む＝(a) writer の例外時 all-or-nothing（M058 がまさに違反＝中途 put しない）／(b) get_or_raise が欠損のみ NotFoundError・他は伝播／(c) exists と get の整合／delete 冪等／(d) 非 CAS 並行（並行 delete/get）等。**fault-injection が要るもの（5xx 伝播等）は fault-injecting ラッパストアを conformancer 同梱**にして wrapper-under-test に被せ検証。M022b の run_middle placeholder を本実装。新しい backend は run_middle を通すだけで実装漏れが loud に落ちる状態を目指す |

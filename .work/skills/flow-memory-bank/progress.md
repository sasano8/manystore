...
| M066 | 挙動契約を全 backend＋client 横断で実行（配線の穴埋め） | **high** | **ユーザー指摘（2026-06-28）＝IF が揃うので同一契約 body を複数実装に流せるはずだが配線が偏っている**。現状: 存在チェックと並行 CAS は全 backend＋Remote(client)＋実 nats/s3(gated) で回るが、**run_light/run_middle・writer all-or-nothing・fail-loud は Dict/Local 止まり**。穴埋め＝(a) **fast CI**: Remote（client→server→local の in-process ASGI）に run_light/middle/writer/fail-loud を流す（外部不要・HTTP 越しシリアライズ/ルーティングの実装漏れを差分検知）／(b) **gated e2e**: 実 nats/s3 にも run_light/middle/writer を流す（`make e2e-up`）。理想＝1 つのパラメタ化ハーネスが {Dict, Local, Remote, 実 nats/s3} に全契約を流す（並行 CAS の現パターンを全契約へ拡張）。fail-loud の実 backend 版は transport fault が要る＝M065 step4 と連動 |

...
| M077 | conformance provider を registry 駆動＋profile 宣言に（ベタ実装削減） | 中 | ユーザー提案（2026-07-02）＝conformance に registry を参照させ、store 構築を `BackendSpec.kv_factory`/`file_factory`（M068）に委ねる。各 provider の construct+connect ベタ実装を、汎用ビルダ `_build_filestore(backend, opts)=get_backend_spec 参照`＋宣言 `BackendProfile(backend, opts, reachable, gated, unsupported)` に圧縮＝**新 backend は「registry 登録＋profile 1 行」で conformance 自動参加**。**線引き**＝構築は registry・**テスト結線（endpoint/creds/tmp dir/reachability/bucket 作成/remote の in-process サーバ/fault 注入）は profile/custom opener に残る**（registry は本番構築情報しか持たない＝ゼロコンフィグ不可）。per-open リソース（local tmp・remote ASGI・fault）は opener hook。`docs/implementing_a_backend.md` の「provider を足す」も profile 方式へ更新。M074 の強化 |

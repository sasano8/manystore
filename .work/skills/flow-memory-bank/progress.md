...
| M027b | **FileStore = KeyValueStore + IO の波及整理**（S3/NATS/HTTP/Safe FileStore に KVS 面を備える） | **未着手・設計先行（相談）** | M027 の follow-up。**Protocol を `FileStore(KeyValueStore)` = KVS + open_reader/open_writer に確定**（ユーザー提案）。これにより各 FileStore は KVS 面（put/get/get_or_raise・iter/list/exists/delete/cp/mv・connect/aclose）まで満たすべきだが、`S3FileStore`/`NatsFileStore`/`HttpFileStore`/`SafeFileStore` は open_reader/open_writer しか持たず**Protocol を部分的にしか満たさない**（`LocalFileStore` は完全準拠済）。CI に型チェッカが無く破綻なし・呼べば AttributeError。選択肢: (i) 各 FileStore に KVS 面を備える（S3/NATS は KVS 兄弟のロジックを流用、HTTP は read-only で mutate 非対応・list/iter は対応可なら追加、Safe は委譲）／(ii) FileStore 用の KVS 面を `KeyValueFileStore`(KVS→FileStore) 経由で得る運用に寄せ各 backend の専用 FileStore を薄く保つ。`KeyValueFromFileStore` を Local 以外の FileStore に被せたい／FileStore を直接 KVS 利用したい需要が出たら着手。projectbrief「最小・汎用/YAGNI」と要相談 |

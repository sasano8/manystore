...
| M078 | 横断関心のミドルウェア合成層（logging/retry/metrics/cache） | 相談 | ユーザー提案（2026-07-02）＝継承/ラップで機能追加する代わりに順序付きミドルウェアを install して op を挟む（ASGI 風）。**適用範囲の線引きが肝**＝(a) **横断的・任意の関心（logging=M015 / op-retry=M014 / metrics / tracing / cache）はミドルウェア向き**（ラッパ乱立回避・productContext「ネスト禁止＝挙動が割れる」の健全な代替＝順序が明示された合成）／(b) **Safe(必須パス検証) と基底の buffered↔stream 合成は対象外**（前者は安全不変条件で opt-out 不可・後者は型の一部）。**設計コスト**＝Store は ~15 の異種メソッドで ASGI の 1 本呼び出しほど綺麗にならない＝per-method フック（薄い・関心少なら十分）か op 封筒（汎用だが型安全喪失）の二択。M071 と直交（統合 Store の上に載る合成層）＝M071 完了後に doc-first。M014/M015 を包含。**追加検討（2026-07-03・ユーザー）＝合成層は 1 機構でなく 3 系統に分かれうる**: **(1) op-middleware**（logging/retry/metrics/cache＝呼び出しを挟む・意味保存＝per-method フック or op 封筒）／**(2) codec エンコーダ・デコーダ層**（json/暗号化/圧縮＝**値そのものを変換**・write で encode/read で decode の対称・**buffered 値経路と streaming FileObject 経路の両方**を覆う必要・順序依存＝compress→encrypt・metrics は平文/暗号文どちらの size を測るか）。**前例＝`manystore/crypto.py`**（`StreamCipher`＋`CipherReader`/`CipherWriter`＝FileObject 境界の codec を**利用側が open_* の戻りを包む**形。store 本体不変＝「真の性能は client wrap」原則）＝codec 層はこの primitive の上に「put/get にも自動適用する薄い Store ラッパ」を足す形が素直／**(3) 認証認可層**: **authn パススルー**（呼び出し側トークンを backend へ透過＝RemoteStore/HTTP/STS 等 remote backend のみ意味を持つ・local/in-proc は透過先が無い）と**store 定義の資格情報モード**（現行の boto/config 委任）を**切替**。要は**per-call の認証コンテキスト**をどう流すか＝現行の最小 IF はメソッドに context 引数を持たない→(a) contextvars で ambient／(b) `store.with_auth(token)` で束縛 view を返す／(c) context 引数を足す（最小 IF 破壊＝YAGNI で却下寄り）。**authz（allow/deny ポリシー）は app 層**＝backend に対応物が無い。**local の認可は OS 権限に無い＝ミドルウェアがポリシーを持つしかない（未定・要検討）**。**論点＝この 3 系統を 1 つの合成機構に載せるか、shape が違うので別レイヤ（M078b codec / M078c auth）に割るか**。私見＝op-middleware だけ M078、codec と auth は shape 差（値変換・ambient context）ゆえ別 doc-first が素直。**詳細設計＝`plans/m078-composition-layers-plan.md`**（2026-07-03・3 系統俯瞰／合成順／M078=op-middleware・M078b=codec・M078c=auth の割り／未決 5 点＝op 形・codec size 意味・codec×整合性メタ・auth per-call 機構・authz スコープ内外） |

...
| M065 | conformancer を「実装漏れ検知器」に育てる（run_middle） | **進行中（high）** | **ユーザー方針＝実装漏れは conformancer に契約として実装し横断検知**。北極星＝conformance を仕様の単一源泉に（テスト可能・pytest-cov 可視・仕様書出力・スキャフォールド材料＝究極の仕様駆動。projectbrief「北極星」）。**step1 完了**＝run_middle 本実装＋絶対契約 `assert_writer_aborts_on_error`（writer all-or-nothing）で M058 を契約先行修正。**step2 完了（2026-06-28）**＝**fault-injection で fail-loud を契約化**＝番兵 `InjectedFault`＋`FaultInjectingKeyValueStore`（全 primitive が障害を投げる「壊れた下層」）＋`assert_fail_loud_propagation`（wrapper が下層障害を None/False/default/NotFoundError に化けさせず伝播するかを get/get(default)/exists/delete/put/list_all/iter_all 横断で検査＝M054/M055 のクラス）。base duality・Safe・KeyValueFileStore・DownloadCache を契約でロック（全て既に健全）＋牙テスト（exists 握り潰し wrapper が落ちる）。conformance 30→35。**残（step3+）**＝(a) Array 等 async 構築 wrapper への適用／(b) leaf backend（nats/s3/remote）への transport-level fault 注入（backend 固有・M055 は httpx.MockTransport 済・nats は要 e2e）／(c) 非 CAS 並行／(d) run_heavy/full／(e) 契約カタログ→spec 文書生成（M034 拡張）・スキャフォールド材料化 |

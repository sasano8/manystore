...
| M065 | conformancer を「実装漏れ検知器」に育てる（run_middle） | **進行中（high）** | **ユーザー方針＝実装漏れは conformancer に契約として実装し横断検知**。北極星＝conformance を仕様の単一源泉に（テスト可能・pytest-cov 可視・仕様書出力・スキャフォールド材料＝究極の仕様駆動。projectbrief「北極星」）。**step1**＝run_middle＋絶対契約 `assert_writer_aborts_on_error`（M058 を契約先行修正）。**step2**＝fault-injection で fail-loud 契約化（`InjectedFault`＋`FaultInjectingKeyValueStore`＋`assert_fail_loud_propagation`＝M054/M055 クラスを wrapper 横断ロック）。**step3 完了（2026-06-28）＝契約カタログ→spec 文書生成（北極星③④）**＝`ContractSpec`＋`ABSOLUTE_CONTRACTS`（絶対契約を id/summary/check で宣言）＋`contract_catalog_drift`/`assert_contract_catalog_current`（カタログの check が実在 assert を指すか＝「仕様だけでテスト無し」を防ぐ drift ガード）＋`differential_contract_aspects`（差分観点を run_* の**実行から導出**＝二重持ちせず乖離しない）。`__main__` が **`docs/conformance_spec.md`** を生成（絶対契約表＋差分観点一覧＋「新 backend の作り方」scaffold 節）。mkdocs nav に追加・`--strict` 緑。conformance 35→38。**残（step4+）**＝(a) Array 等 async 構築 wrapper／(b) leaf backend の transport-level fault（nats/s3/remote・M055 は httpx.MockTransport 済・nats は要 e2e）／(c) 非 CAS 並行／(d) run_heavy/full／(e) カタログから backend 雛形を実生成（scaffold 自動化＝④の完全形） |
...
| M066 | 挙動契約を全 backend＋client 横断で実行（配線の穴埋め） | **進行中（high）** | **ユーザー指摘＝IF が揃うので同一契約 body を複数実装に流せるはずだが配線が偏っている**。**step1 完了（2026-06-28）**＝(a) **fast CI**: Remote（client→server→local の in-process ASGI）に **run_light/run_middle/writer all-or-nothing** を流す＝`KeyValueFileStore(RemoteKeyValueStore)` で FileStore 化し HTTP 往復の IO も差分・絶対契約で検証（test_client +3・全緑＝シリアライズ/ルーティングの実装漏れ無し）。(b) **gated e2e**: `test_backend_writer_aborts_on_error` を全 CASE（local 常時/nats・s3 gated）に追加＝実 backend で writer 契約を検証（自分の uuid キーのみ＝非破壊）。**残（step2）**＝① 実 backend への run_light/run_middle 適用（`delete_all` がストア全体を消す＝専用テスト名前空間 or 非破壊版 run_* の設計が要る）／② Remote への fail-loud（HTTP 越しは障害型が変わる＝server 側 fault 注入＋client が raise する形＝M065 step4 連動）／③ native FileStore（S3 streaming 等）の writer も直接検証（現状は KVS バッファ writer 経由）／④ 1 パラメタ化ハーネスへ集約 |

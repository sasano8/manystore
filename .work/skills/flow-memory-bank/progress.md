...
| M025 | 名前空間を buffer 性で再編（`kv`=buffered / `storage`=streaming）＋ bucket/path addressing | **フェーズ1（移設）完了 / 改訂版（addressing・prefix 撤去）＝設計確定・未着手 / フェーズ2・3 残** | **ユーザー要望（2026-06-23 初版 / 2026-06-24 改訂, 対話）**。設計 `m025-namespace-restructure-plan.md`（改訂版）。第1階層を buffer 性で分ける＝`kv/*`(バッファ) / `storage/*`(ストリーミング)。4 ルート: `kv/raw`(既存) / `kv/json`(新規=JSON 検証) / `storage/s3`(既存=S3 GW) / `storage/manystore`(新規=FileStore streaming)。<br>**フェーズ1 移設 完了（2026-06-23）**: combined の prefix を `/manystore`→`/kv/raw`・`/s3`→`/storage/s3` に付け替え。`make check` 緑（91/1）。<br>**改訂（2026-06-24・M028 を受けて）= M025改 として設計確定**: addressing を `contexts/{ctx}/objects/{key}` → **`NS/{bucket}/{path}`**（context=ArrayStorage 第一階層＝bucket、後続は不透明 `{path}`、`contexts`/`objects`/`keys` 飾り廃止、表層語を bucket に統一）。コレクション判別＝空パス=一覧/非空=オブジェクト（prefix 廃止で末尾スラッシュ規則不要）。WS イベントは同パス `WS NS/{bucket}/` を upgrade で判別。**native HTTP から prefix を撤去**（フラット list-all。prefix は M030 の capability へ移設）。破壊的変更（未リリース＝互換エイリアス無し）。触る: `server/routes.py`・`combined.py`・`client/remote.py`・`server/static/app.js`（フラット list-all+クライアント畳み）・`tests/ui/*`・README/examples。<br>**残**: M025改（addressing）／M030（prefix capability・先行 or 同時）／フェーズ2 `kv/json`／フェーズ3 `storage/manystore`（range/chunked）。priority normal |

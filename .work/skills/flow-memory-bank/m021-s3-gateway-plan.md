...
| B 最小+multipart | A + CreateMultipartUpload/UploadPart/Complete/Abort | 大容量 PUT に必要。**S2 で実装済（2026-06-23）**＝ただし `S3FileStore` writer 経由ではなく**コア IF 不変**で `StorageService` の上に薄く合成（予約キー空間）。ListParts/ListMultipartUploads は YAGNI 見送り |
...
- **S2 multipart PUT（optional・大容量）**: ✅ **完了（2026-06-23）**。当初案（`S3FileStore` の
  multipart writer に橋渡し）は **採らず**、**コア IF 不変**で `StorageService.put/get/delete/list_entries`
  の上に薄く合成した（projectbrief「最小・汎用／IF を安易に拡張しない」を死守）。
  - 実装ファイル: `manystore/gateway/multipart.py`（新）／`manystore/implement/s3map.py`（multipart XML 補助を追加）／
    `manystore/gateway/routes.py`（PUT/POST/DELETE の query 多重化分岐を追加）／`manystore/gateway/__init__.py`（docstring）。
  - **状態の保持方式 = ストア上の予約キー空間** `.manystore-mpu/{uploadId}/{partNumber:05d}`（インメモリ辞書では
    なく）。理由: **サーバ再起動耐性・複数プロセス/ワーカ耐性**（落ちても part が残り、どのワーカでも同じストアへ積む）。
    予約プレフィクスは `validate_safe_path` を通る安全キー。**ListObjectsV2 は予約プレフィクスを除外**（gateway 側で
    フィルタ＝service 不変）。
  - 対応 API: CreateMultipartUpload（`POST ?uploads`・uploadId=uuid4 hex 発行）／UploadPart（`PUT ?partNumber=N&uploadId=X`・
    一時キーへ put・part の MD5 を ETag で返す・**同一 part 再 put は last-writer-wins**）／CompleteMultipartUpload
    （`POST ?uploadId=X`＋本文の Part 列・**指定順**で結合→本オブジェクトへ **1 回の put（all-or-nothing）**→一時 part 掃除・
    ETag=`<concat-md5>-N`）／AbortMultipartUpload（`DELETE ?uploadId=X`・一時 part 全削除・冪等）。
  - **見送り（YAGNI・バックログ）**: ListParts / ListMultipartUploads。
  - コア IF 変更: **なし**（`KeyValueStore`/`FileStore`/`StorageService` 公開 API いずれも不変）。
  - 受け入れ: ✅ 複数 part の multipart upload→GET 一致／abort 経路を**実 aiobotocore クライアント往復**で検証
    （`tests/ui/test_gateway_s3client.py` +2）。in-process route 分岐（`test_gateway.py` +5）・XML 純ロジック
    （`test_s3map.py` +4）も追加。✅ `make check` 緑（**87 passed, 1 skipped**／S1 の 76 から +11）。

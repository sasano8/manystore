...
**M071（公開 1 Store 統合）完遂（2026-07-03）＝Stage 1〜6＋語彙 purge まで完了。次サイクル未着手＝候補から選定**
（詳細は下記「直近の変更」冒頭・progress M071 行）。以下は M071 の経緯（履歴）。
...
- **M071 完了（2026-07-03）＝語彙 purge で「徹底統合」を締め＝旧「KeyValueStore/FileStore」語彙を言語レベルまで一掃**
  （ユーザー要望・旧名 alias 完全撤去＋docstring 書き換え両「推奨」承認）。5 独立コミット: ①合成/surface を Store 名へ
  （Safe/IPFS 各1クラス統合・Array/AsyncToSync/LoadBalanced/connect_store）②backend 旧名 alias を shim なし完全撤去
  ③アダプタ KeyValueFileStore/KeyValueFromFileStore と生 factory create_unsafe_{kv,file}_store 撤去（合成は基底の既定へ・
  入口 create_unsafe_store 一本）④conformancer 改名（StoreTester/assert_store/assert_buffered_store/FaultInjectingStore）
  ⑤docstring・手書き docs・生成 docs を 1-Store モデルへ（architecture.md 全面改稿・散文は 3 subagent 並行→中央 diff
  レビュー）。最終 make check 緑（268）・test-heavy 緑（44・実 backend）・mkdocs --strict 緑。**M071 は Stage 1〜6＋語彙 purge まで完遂＝公開面は完全に 1-Store**。
  次サイクル候補＝M051（k8s secrets backend）／M076（nats-fake）／M078（横断関心ミドルウェア合成層）。
  - **M071 語彙 purge の取りこぼし是正（2026-07-03）＝トップレベル文書**: 初回 purge は manystore/・tests/・docs/*.md に
    偏り、**README.md（`manystore.kv`/`file` facade・`connect_key_value_store` 等＝サンプルの import が壊れていた）**・
    examples/*.toml コメント・`url.py`/conformancer の docstring・**tests のテスト関数名（`*_file_store_*`/`*_key_value_*`）**を
    見落としていた。README を現行 API（顔＝`open_async_store`・`SafeStore`・`create_*_store`）へ全面改稿し実走確認、
    テスト名も Store 語彙へ改名。リポジトリ全体 sweep で非履歴の旧語彙 0 を確認。make check 緑（268）。教訓＝**purge は
    src だけでなく README/examples/生成物までリポジトリ全体を最初に grep すべき**。

- **M071 facade 統合を完了＝`storage/file.py`・`storage/kv.py` を削除**（2026-07-02）。統合入口の live 実装
  （`open_store`/`open_async_store`/`create_safe_store`/`_resolve_context`/array 系）を `storage/store.py` へ引き上げ、
  live 二次 API（ArrayKeyValueStore/StorageMirror/DownloadCache/connect 系）も store へ集約。トップ `manystore` は
  `store.__all__` のみをフラット公開＝**公開面を 1-Store に整理**。非推奨エイリアス（`manystore.kv`/`manystore.file`
  名前空間・`*_key_value_store`/`*_file_store` factory・`DictFileStore`/`S3KeyValueStore` 等の旧名）はクリーンに撤去。
  依存はテストのみ（src コードは非依存）＝test_storage/test_conformance ほか 9 ファイルを新名へ機械移行（Safe/adapter は
  surfaces.safe / spec へ repoint）。conformancer src も新名へ・docs 再生成・architecture.md 更新。make check 緑（271）・
  test-heavy 緑（44）・mkdocs --strict 緑。
- **registry を storage 直下へ移設**（2026-07-02）＝`storage/backends/registry.py`→`storage/registry.py`。理由＝registry は
  「名前→ストア生成」の**解決レイヤ**で url/config/connect/store と同列＝具体 backend の置き場（backends/）の兄弟ではなく
  一つ上。依存は `spec` のみ（循環なし）。`backends/__init__` は `from ..registry` で seed＋再エクスポート継続＝公開 API 不変。
  test は `from manystore.storage import registry`。make check 緑（271）・test-heavy 緑（44）。

...
**M074 完了（2026-07-02）＝conformance real/fake/fault 切替＋backend 実装ガイドライン**（s3-fake を非 gated
provider 化で docker 無し fast に全契約／CAS は非権威 xfail＝認証は実 backend+決定的テスト／`docs/implementing_a_
backend.md`）。nats-fake は JetStream メタ subject 忠実化が要り **M076 へ follow-up**。次サイクル候補＝**M051（k8s
secrets backend＝当初 5 要望の最後）** ／ M076（nats-fake）／ 大型 doc-first の M071＋M073（公開 IF 統合＋仕様集約）。

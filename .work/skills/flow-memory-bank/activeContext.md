...
- **【要注意】既存 flaky を発見＝M072**：`concurrent_delete_safe[local]` が fast フルスイートで ~1/8 落ちる（local 並行
  delete/get レースで生 FileNotFoundError 漏れ・M068/M013 と無関係の既存バグ）。「既知の問題」参照。次サイクル候補。

...
conformance 25→38。
**M066 step1 完了（2026-06-28）＝挙動契約の backend/client 横断配線**＝(a) fast CI で Remote（in-process
ASGI）に run_light/run_middle/writer を流す（`KeyValueFileStore(Remote)` で FileStore 化・HTTP 往復を差分検証・
test_client +3）／(b) gated e2e に `test_backend_writer_aborts_on_error` を全 CASE 追加（非破壊）。残＝実 backend
への run_*（delete_all 破壊性の設計）・Remote fail-loud（M065 step4 連動）・native writer 直接検証。
最優先 = **M066 step2** or **M065 step4**（scaffold 自動化／transport fault）/**M056**（nats 無ロック）/

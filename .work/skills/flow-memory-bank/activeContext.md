...
**M069 完了（2026-07-02）＝名前 URL でストアを開く**（`open_store(url)`・fsspec 風・M068 registry の上）。
次は **M070（`manystore store init`＋構成ファイルからストア復元）**：上方向 discovery・config 基準の相対パス解決・
`serving/services/config.py` パーサを neutral へ昇格し client 共有。`open_store("ctx")` の名前解決も M070 で。
続いて M051（k8s）。M071（公開 IF 統合＝`BufferedStore`/`StreamingStore`）と M073（仕様集約）は大型 doc-first で後。
M074（conformance を real/fake/fault 切替＝fake を provider 化）は品質系 backlog。詳細は progress「残作業」。

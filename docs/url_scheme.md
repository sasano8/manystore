...
`open_store(url)` は既存の顔 `open_async_store` の URL 版＝**Safe 包装＋接続 CM**
...
open_store(url: str, *, verify: bool = True, policy: ConnectPolicy | None = None)
    -> async context manager[SafeStore]   # Safe 包装＋接続
...
`open_async_store(backend, **opts)` に委譲する。**scheme が無ければ構成ファイルの context 名**
...
- IO API（`open_*`）を明示的に開く別サーフェスと名前解決（`open_store("ctx")`）は後続（M070 連動）。

...
> 準拠テスト（`manystore.conformancerr` / `tests/test_conformance.py`）。
...
サードパーティ backend は `manystore.conformancer` で検査できる。2 段階:
...
```python
from manystore.conformancer import assert_key_value_store, assert_file_store
...
from manystore.conformancer import FileStoreTester, save_report

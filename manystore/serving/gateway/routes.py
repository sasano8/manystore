...
from ...spec import FileInfo, IfMatch
from ...spec.exceptions import ConflictError
...
class _Precondition(Exception):
    """conditional write の precondition 不成立（→ 412 PreconditionFailed）。"""


class _Unsupported(Exception):
    """非対応の conditional 形（→ 501 NotImplemented）。"""
...
def build_router(service: StorageService):
    ...
    def _precondition_failed(resource: str) -> Response:
        # If-Match / If-None-Match 不成立は S3 では 412 PreconditionFailed。
        return _xml(
            412,
            render_error(
                code="PreconditionFailed", message="precondition failed", resource=resource
            ),
        )

    async def _resolve_condition(bucket: str, key: str, headers) -> IfMatch:
        """S3 conditional write ヘッダを backend の `if_match`（[IfMatch]）へ解く。

        - `If-None-Match: *` → create-only（不在 FileInfo・既存なら backend が `ConflictError`）。
        - `If-Match: *` → 存在を要求する update（不在は precondition 不成立）。
        - `If-Match: "<etag>"` → 現 ETag（=本体 MD5）一致を要求し、**backend の version トークンで
          原子性を担保**（read と put の間の並行変更は backend CAS が `ConflictError` で弾く）。

        precondition 不成立は `_Precondition`、非対応形は `_Unsupported`。gateway の ETag は MD5
        （S3 互換・既存契約）で backend の CAS トークンと別物ゆえ、If-Match は MD5 突合（S3 意味論）
        ＋ head の version 渡し（原子性）の二段で写す。
        """
        if_none_match = headers.get("if-none-match")
        if if_none_match is not None:
            if if_none_match.strip() != "*":
                raise _Unsupported  # S3 PutObject の If-None-Match は `*` のみ
            return FileInfo.absent(key)
        if_match = headers.get("if-match")
        if if_match is not None:
            base = await service.head_or_absent(bucket, key)
            if base.is_absent():
                raise _Precondition  # If-Match 指定だが対象が無い
            want = if_match.strip()
            if want != "*":
                current = await service.get_or_raise(bucket, key)
                if _etag(current) != want:
                    raise _Precondition  # ETag（本体 MD5）不一致
            return base  # update（version トークンで原子 CAS）
        return None  # 無条件 PutObject（LWW）
    ...
    async def put_object(bucket: str, key: str, request: Request) -> Response:
        ...
        # conditional write（S3 conditional PUT）＝If-None-Match: * / If-Match: <etag|*>。
        try:
            cond = await _resolve_condition(bucket, key, request.headers)
        except _Precondition:
            return _precondition_failed(resource)
        except _Unsupported:
            return _xml(
                501,
                render_error(
                    code="NotImplemented",
                    message="If-None-Match supports only '*'",
                    resource=resource,
                ),
            )
        except Exception as exc:
            return _error_response(exc, resource)
        try:
            await service.put(bucket, key, body, if_match=cond)
        except ConflictError:
            return _precondition_failed(resource)  # backend CAS 不一致＝precondition 不成立
        except Exception as exc:
            return _error_response(exc, resource)

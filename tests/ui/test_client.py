...
from manystore import DictFileStore, KeyValueFileStore
...
# ── 挙動契約を Remote 経由でも流す（fast CI・in-process ASGI で外部不要）・M066 ──
#
# IF が揃うので Dict/Local と同じ契約 body を client→server→local の HTTP 往復に流す。
# RemoteKeyValueStore（KVS）を KeyValueFileStore で FileStore 化し、open_reader/open_writer の
# IO も HTTP 越しに回す＝シリアライズ/ルーティングの実装漏れを差分・絶対契約で横断検知する。
# （fail-loud は HTTP 越しで障害の型が変わる〔InjectedFault→HTTPStatusError〕＝別途 transport
#  fault が要る M065 step4 の領分なのでここでは流さない。）


async def test_remote_run_light_matches_oracle(tmp_path: Path) -> None:
    # 辞書ストアを正に、Remote 越し FileStore の IO/exists/list_all/iter_all を差分検証。
    service, store = await _remote_store(tmp_path)
    try:
        tester = FileStoreTester(DictFileStore(), KeyValueFileStore(store))
        report: list = []
...
async def test_remote_run_middle_matches_oracle(tmp_path: Path) -> None:
    # delete/冪等/複数キー/read 境界/overwrite 縮小の細かい契約も Remote 越しに差分検証。
    service, store = await _remote_store(tmp_path)
    try:
        tester = FileStoreTester(DictFileStore(), KeyValueFileStore(store))
        report: list = []
...
async def test_remote_writer_aborts_on_error(tmp_path: Path) -> None:
    # 絶対契約: Remote 越しでも writer の例外時に中途バッファを確定しない（サーバ側にキー不作成）。
    service, store = await _remote_store(tmp_path)
    try:
